...
def parse(dirname, out_dir):
	...
	for filename in fs:
		with codecs.open(dirname + "/" + filename, "r", "shift_jis") as f:
			print(filename)
			MODE_READING = 1
			MODE_END = 2
			sentences = []
			...
			for line in f:
				...
				if re.search(r"^[0-9]{4}", line):
					if len(sentences) > 0 and skip == False:
...
def main(args):
	...
	# parse(trn_dir + "/core", args.out_dir + "/core")
	parse(trn_dir + "/noncore", args.out_dir + "/noncore")
